...
/**
 * GET /api/github/verify-repo
 * query: repoFullName=owner/repo
 * checks if repo exists + accessible
 */
router.get("/verify-repo", async (req, res) => {
  try {
    ...
    return res.status(status).json({
      success: false,
      message: err.response?.data?.message || "Repo verification failed",
    });
  }
});
